  ...
  if (first) {
    achievements.push({
      ...
      description: pickVariant(`live-first-${question.id}-${first.id}`, [
        ...
        `${first.name} влетел в ответ до того, как трек успел представиться.`,
        `${first.name} нажал так быстро, будто кнопка сама попросила.`,
        `${first.name} первым поставил репутацию на кон.`
      ]),
    ...
    achievements.push({
      ...
      description: closeToFirst
        ? pickVariant(`live-second-close-${question.id}-${second.id}`, [
            ...
            `${second.name} почти украл первую кнопку, но не хватило миллиметра.`,
            `${second.name} включил турборежим, но кто-то был еще нервнее.`
          ])
        : pickVariant(`live-second-${question.id}-${second.id}`, [
            ...
            `${second.name} ответил быстро, но история любит первых.`,
            `${second.name} взял серебро в дисциплине "тыкнуть и надеяться".`
          ]),
  ...
  if (late) {
    achievements.push({
      ...
      description: pickVariant(`live-late-${question.id}-${late.id}`, [
        ...
        `${late.name} тянул до последнего, будто ответ начисляет проценты.`,
        `${late.name} вспомнил про игру почти вовремя.`,
        `${late.name} нажал на грани уважения к таймеру.`
      ]),
      recipient: late.name,
      tone: 'safe',
      weight: 58,
  ...
  if (answers.length === room.players.size && room.players.size > 1) {
    achievements.push({
      ...
      description: pickVariant(`live-all-${question.id}`, [
        ...
        'Комната закончила тыкать. Сейчас начнется бухгалтерия стыда.',
        'Ответы собраны. Осталось понять, кто тут артист.',
        'Все отметились. Коллективная уверенность выглядит подозрительно.'
      ]),
  ...
  if (topOption && room.players.size >= 3 && topOption.count >= Math.max(3, Math.ceil(answers.length * 0.6))) {
    achievements.push({
      ...
      description: pickVariant(`live-majority-${question.id}-${topOption.optionId}`, [
        ...
        'Комната нашла общий вариант. Теперь он отвечает за всех.',
        'Большинство пошло в одну дверь. Табличку на двери пока не видно.',
        'Толпа уверенно сгруппировалась. Это может быть как гений, так и беда.'
      ]),
  ...
  if (topOption && secondOption && answers.length >= 4 && topOption.count - secondOption.count <= 1 && topOption.count + secondOption.count >= answers.length * 0.75) {
    achievements.push({
      ...
      description: pickVariant(`live-split-${question.id}`, [
        ...
        'Ответы разошлись по лагерям. Музыка смотрит на это молча.',
        'Комната не договорилась. Демократия снова делает больно.',
        'Половина против половины. Кто-то сейчас будет очень убедительно неправ.'
      ]),
      recipient: 'Комната',
      tone: 'chaos',
      weight: 54,
  ...
  const steps = [
    {
      ...
      description: [
        ...
        `${player.name} сделал первый разворот. Навигатор молчит.`,
        `${player.name} слегка поправил курс. Уже подозрительно.`,
        `${player.name} сменил мнение. Бывает. Пока бывает.`
      ],
      weight: 60
    },
    {
      ...
      description: [
        ...
        `${player.name} передумал снова. Уверенность вышла покурить.`,
        `${player.name} ищет ответ методом маршрутки: где остановят, там выйдет.`,
        `${player.name} добавил второй разворот. Стабильность переоценена.`
      ],
      weight: 70
    },
    {
      ...
      description: [
        ...
        `${player.name} проверяет гипотезы быстрее, чем они появляются.`,
        `${player.name} открыл меню дегустации ответов.`,
        `${player.name} уже не играет, а проводит аудит кнопок.`
      ],
      weight: 80
    },
    {
      ...
      description: [
        ...
        `${player.name} устроил кнопкам стресс-тест и себе тоже.`,
        `${player.name} сменил столько, что ответ уже должен платить аренду.`,
        `${player.name} перешел из режима выбора в режим шаманства.`
      ],
      weight: 92
    }
  ];
